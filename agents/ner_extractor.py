...
# Types NER reconnus par AnonyNER — doit rester synchronisé avec coordinator/models.py
NER_LABELS = {
...
_MODEL_PATH = Path(__file__).parent.parent / "models" / "anonyner_model" / "model-best"
...
class NERExtractor:
    ...
    - N'anonymise pas : retourne les valeurs originales (le coordinateur
      peut activer l'anonymisation séparément via AnonyAgent)
    """
    ...
    def extract(self, text: str) -> dict[str, list[str]]:
        ...
        Args:
            text: Texte brut (description de tâche, message utilisateur, etc.)
        ...
        """
        # Initialiser avec toutes les clés vides
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_ in NER_LABELS:
                    ...
                    val = ent.text.strip()
                    if val and val not in lst:   # déduplication ordre-préservé
                        lst.append(val)
        except Exception as exc:
            logger.warning("NERExtractor.extract() error: %s", exc)
